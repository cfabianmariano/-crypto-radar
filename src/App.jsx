...
function CandleChart({ data, signals = [] }) {
  ...
  const geometry = useMemo(() => {
    ...
    const candleWidth = Math.min(11, Math.max(1.5, (plotW / data.length) * 0.66))
  ...
  const xTickIndexes = Array.from(new Set([0, Math.floor((data.length - 1) * .25), Math.floor((data.length - 1) * .5), Math.floor((data.length - 1) * .75), data.length - 1]))
  ...
  const handlePointer = (event) => {
    ...
    setHoverIndex(Math.max(0, Math.min(data.length - 1, Math.floor(ratio * data.length))))
  }

  return (
    <div className="simpleCandleBlock">
      ...
      <div className="candleCanvas" ref={wrapRef} onPointerMove={handlePointer} onPointerLeave={() => setHoverIndex(null)}>
        <svg viewBox={`0 0 ${g.W} ${g.H}`} role="img" aria-label="Gráfico de velas BTC">
          ...
          {xTickIndexes.map((i) => <text key={i} x={g.x(i)} y={g.H - 10} textAnchor="middle" fill="#75849a" fontSize="12">{data[i]?.date}</text>)}
        </svg>
    ...
    <main className="appShell">
      ...
      {btcModel && (
        <section className={`decisionCard glass ${actionClass}`}>
          ...
          <div className="decisionMetrics simpleDecisionMetrics">
            <div><span>Compra histórica</span><strong>{btcModel.buyStats.hitRate == null ? '—' : `${(btcModel.buyStats.hitRate * 100).toFixed(0)}%`}</strong><small>{btcModel.buyStats.n} casos</small></div>
            <div><span>Venta histórica</span><strong>{btcModel.sellStats.hitRate == null ? '—' : `${(btcModel.sellStats.hitRate * 100).toFixed(0)}%`}</strong><small>{btcModel.sellStats.n} casos</small></div>
          </div>
        </section>
      )}
        ...
        <>
          <section className="chartCard glass simpleChartCard">
            <div className="sectionTitle chartHeader">
              <div><span className="eyebrow">BTC</span><h2>Precio y señales</h2></div>
          ...
          {coinId !== 'bitcoin' && (
            <>
              <section className="scoreGrid"><Gauge value={indicators.bottomScore} label="BOTTOM SCORE" subtitle={scoreLabel(indicators.bottomScore, 'bottom')} tone="green" /><Gauge value={indicators.trendScore} label="TREND SCORE" subtitle={scoreLabel(indicators.trendScore, 'trend')} tone="blue" /></section>
              <section><div className="sectionTitle outside"><div><span className="eyebrow">DIAGNÓSTICO</span><h2>Indicadores actuales</h2></div></div><div className="indicatorsGrid">{indicators.rows.map((item) => <IndicatorCard key={item.label} item={item}/>)}</div></section>
            </>
          )}
        </>
      )}
      ...
      <footer>BTC V0.5 · señal, efectividad y gráfico.</footer>
    </main>
  )
}
